...
plots = []
for line in open('plots_to_magnet.txt'):
  if line.startswith('#'):
    continue
  else:
...
updates = init_list_of_objects(2)
batch_loss = init_list_of_objects(2)
...
with open(os.getcwd() + plots[fnum],'r') as csvfile:
    ...
    for row in variables:
    	if i >= 29:
			...
			batch_loss[fnum].append(float(row[1]))
...
#plt.plot(updates[0], batch_loss[0], '-b', label="LeNet - Magnet Loss")
plt.plot(batch_loss[0], 'c', label="LeNet")
plt.plot(moving_average(batch_loss[0], 20), 'b')
...
plt.savefig(args.title + "_magnet_trainloss.pdf", bbox_inches='tight')
plt.close()
